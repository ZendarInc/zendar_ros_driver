...
nav_msgs::OccupancyGrid RangeMarkers(const zpb::tracker::message::TrackerState& tracker_state, const float max_range){
    ...
    // Define the width, and height of the image (in pixel). This way of computing the number of pixels is done
    // since for a max_range of 40, a 1001x1001-pixel image provides a decent resolution, and all other values
    // such as the offset of the text to the range marker were choosen based on this resolution.
    int width = max_range / (float) 40 * 1001;
    int height = max_range / (float) 40 * 1001;
    ...
    // Because ros/webviz flips the colors, we draw black circles on a white image
    ...
    for (int i = 1; i < num_circles + 1; i++){
      ...
      cv::Point image_center_shifted = cv::Point(image_origin_y - 50, image_origin_x - circle_radius - 20);
      float displayed_range = round(i * max_range / num_circles * (float) 100.0) / (float) 100.0;
      // Set display format for range
      std::stringstream stream;
      stream << std::fixed << std::setprecision(2) << displayed_range << " m";
      cv::putText(img, stream.str(), image_center_shifted, cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0), 3);
    }
